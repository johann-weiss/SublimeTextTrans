...
if sublime.platform() == 'windows':
    ...
    # default global variables , needed to use plugin_loaded function in order
    # to work on ST3
    stt_opacity = 0
    stt_autoapply = False
    stt_level0 = 0
                ...
                if (LHParent == 0) or (LHParent == LHDesktop):
                    if(classs == b'PX_WINDOW_CLASS'):
                        ...
                        try:
                            ...
                            ShellExecute(
                                LHDesktop, "open", exe_file, parametro, None, SW_HIDE)
                            if opacity is not None:
                                SetLayeredWindowAttributes(
                                    LHWindow, 0, opacity, LWA_ALPHA)
                            break
                        except ValueError:
                            print("Error! ")
    ...
    def plugin_loaded():
        ...
        stt_level1 = settings.get('level1', 170)
        stt_level2 = settings.get('level2', 180)
        stt_level3 = settings.get('level3', 190)
        stt_level4 = settings.get('level4', 200)
        stt_level5 = settings.get('level5', 210)

        # Python fails calling SetWindowLong from Windows and crashes the
        # entire Sublimetext, so we will use an exe file to set layered mode
        # the sublimetext running app
